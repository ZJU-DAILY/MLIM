...
class GraphEnvironment:
    ...
    def reset(self):
        ...
        self.is_selected = {key: -1 for key in self.top_tenpct_nodes}  
        ...
        self.embedding_test_archive = self.embedding_archive_0time.detach().clone() 
        self.embedding_train_archive = self.embedding_archive_0time.detach().clone() 
    ...
    def test_step(self, action, step):
        ...
        if (step + 1) not in embedding:
            embedding[step + 1] = self.embedding_test_archive.clone() 
    ...
    def get_environment_embedding(self, step):
        ...
        if selected_embs.shape[0] > 0:
            mu_selected = torch.amax(selected_embs, dim=0) 
