...
from QNet_model.utils_dqn import calculate_spread
...
def main():
    ...
    proc = subprocess.Popen(
        ['/home/cxq/lrz/GNN-QNet-20250808/QNet_model/rr_server', f'-dir={dataset_path}'],
        # 输入种子节点集合, 用rr_server计算节点的socre
    ...
    #pred_path = os.path.join(script_dir, "../pred_score_p", f"{qnet_config.DATASET}-{R}-60-{qnet_config.ratio}-predict-score.txt")
    print(f"pred_path: {pred_path}")

    graph_env = GraphEnvironment(qnet_config, dataset_path,pred_path, use_gts,p)
    agent = Agent(qnet_config)
    
    train(qnet_config, graph_env, agent, proc)
